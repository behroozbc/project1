...
class EncoderPart(nn.Module):
    def __init__(self,inChannel:int,outChannel:int,kernel=(5,3),pardding=(2,1)):
        super().__init__()
...
class DecoderPart(nn.Module):
    def __init__(self, inChannel:int,outChannel:int,outputPadding=0,kernel=(5,3)):
        super().__init__()
        self.seq1=nn.Sequential(
            nn.ConvTranspose2d(inChannel,outChannel, kernel_size=kernel, stride=2, output_padding=outputPadding),
            nn.BatchNorm2d(outChannel),
            nn.ELU())
    def forward(self,x):
        out1 = self.seq1(x)
...
class Decoder(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.deconv1 = DecoderPart(256,128,kernel=3,outputPadding=(0, 1))  # (256, 9, 2) -> (128, 17, 4)
        self.deconv2 = DecoderPart(128,64,kernel=3) # (128, 17, 4) -> (64, 33, 7)
        self.deconv3 = DecoderPart(64,32,kernel=3)                           # (64, 33, 7) -> (32, 65, 13)
        self.deconv4 = DecoderPart(32,16,outputPadding=(0,1))    # (32, 65, 13) -> (16, 129, 26)
        self.deconv5 = DecoderPart(16,1)                            # (16, 129, 26) -> (1, 257, 51)
...
writer = SummaryWriter(log_dir='./runs/'+nowTime+'2-simple')
