    ...
    def __init__(self):
        ...
        self.conv1 = nn.ModuleList([
            nn.Sequential(
                ...
                nn.BatchNorm2d(16),
                nn.ELU(),
                nn.MaxPool2d(2)                              # (16, 201, 4) -> (16, 100, 2)
            ) for _ in range(3)
        ])
        ...
        self.conv2 = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(48, 32, kernel_size=3, padding=1), # (48, 100, 2) -> (32, 100, 2)
                nn.BatchNorm2d(32),
                nn.ELU(),
                nn.Conv2d(32, 32, kernel_size=3, padding=1), # (32, 100, 2) -> (32, 100, 2)
                nn.BatchNorm2d(32),
                nn.ELU(),
                nn.MaxPool2d(2)                              # (32, 100, 2) -> (32, 50, 1)
            ) for _ in range(3)
        ])
        
        # Third part: Linear layer for bottleneck
        self.fc = nn.Linear(96 * 50 * 1, 256)  # 4800 -> 256
        
    def forward(self, x):
        # First parallel convolutions
        outs1 = [branch(x) for branch in self.conv1]  # 3 x (batch_size, 16, 100, 2)
        out1 = torch.cat(outs1, dim=1)                # (batch_size, 48, 100, 2)
        
        # Second parallel convolutions
        outs2 = [branch(out1) for branch in self.conv2]  # 3 x (batch_size, 32, 50, 1)
        out2 = torch.cat(outs2, dim=1)                   # (batch_size, 96, 50, 1)
        
        # Flatten and apply linear layer
        out2_flat = out2.view(out2.size(0), -1)  # (batch_size, 4800)
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(256, 96 * 50 * 1)  # 256 -> 4800
        
        # Second part: Three parallel deconvolutional layers with 32 filters
        self.deconv1 = nn.ModuleList([
            nn.Sequential(
                nn.Upsample(size=(100, 2)),            # (96, 50, 1) -> (96, 100, 2)
                nn.Conv2d(96, 32, kernel_size=3, padding=1), # (96, 100, 2) -> (32, 100, 2)
                nn.BatchNorm2d(32),
                nn.ELU(),
                nn.Conv2d(32, 32, kernel_size=3, padding=1), # (32, 100, 2) -> (32, 100, 2)
                nn.BatchNorm2d(32),
        ...
        self.deconv2 = nn.ModuleList([
            nn.Sequential(
                nn.Upsample(size=(201, 4)),            # (96, 100, 2) -> (96, 201, 4)
    ...
    def forward(self, z):
        ...
        out = out.view(-1, 96, 50, 1)         # (batch_size, 96, 50, 1)
        
        # First parallel deconvolutions
        outs1 = [branch(out) for branch in self.deconv1]  # 3 x (batch_size, 32, 100, 2)
        out1 = torch.cat(outs1, dim=1)                    # (batch_size, 96, 100, 2)
        
        # Second parallel deconvolutions and averaging
        outs2 = [branch(out1) for branch in self.deconv2] # 3 x (batch_size, 1, 201, 4)
...
num_epochs = 100
enc=Encoder()
writer = SummaryWriter(log_dir='./runs/'+datetime.now().strftime("%m-%d-%Y--%H-%M_")+'paraller-autoencoder-deeper')
