...
from BaseFunc import plotit  # Assuming plot_latent_spaceVAE is not directly applicable
...
dataset = CustomDataLoader(rootDirs=['E:\\Work\\University\\PR\\datas\\voice_gender_detection\\females', 'E:\\Work\\University\\PR\\datas\\voice_gender_detection\\males'], sr=16000, duration=0.5, HaveSaveOutput=False, isNormalized=True, randomSelection=False)
...
torch.save(vqvae.state_dict(), nowTime + '_vqvae.pth')

# Plot Codebook Usage
vqvae.eval()
all_indices = []
with torch.no_grad():
    for spec, _ in valloader:
        spec = spec.cuda()
        _, _, _, indices = vqvae(spec)
        all_indices.append(indices.cpu())
all_indices = torch.cat(all_indices, dim=0)  # (num_samples, 34)
codebook_usage = torch.bincount(all_indices.view(-1), minlength=512)
plt.figure(figsize=(10, 6))
plt.bar(range(512), codebook_usage.numpy())
plt.title("Codebook Usage Histogram")
plt.xlabel("Codebook Index")
plt.ylabel("Frequency")
plt.savefig(nowTime + '_codebook_usage.png')
plt.close()

# SVM Classification on Latent Space
latent_features = []
labels_list = []
with torch.no_grad():
    for spec, label in valloader:
        spec = spec.cuda()
        _, _, _, indices = vqvae(spec)
        for idx in indices:
            hist = np.bincount(idx.cpu().numpy(), minlength=512)
            latent_features.append(hist)
        labels_list.extend(label.numpy())
latent_features = np.array(latent_features)
labels_list = np.array(labels_list)

clf = svm.SVC(kernel='rbf')
clf.fit(latent_features, labels_list)
pred = clf.predict(latent_features)
accuracy = accuracy_score(labels_list, pred)
print(f'SVM Classification Accuracy on Validation Set: {accuracy:.4f}')
